...
const Carousel = forwardRef(
	({ children, className }: Props, ref: Ref<CarouselRef>) => {
		...
		const scrollLeft = () => {
			const newIndex =
				featureIndex === 0 ? featureLength - 1 : featureIndex - 1;
		...
		const handlePointerUp = () => {
			...
			const scrollDiff =
				carouselRef.current.scrollLeft - mouseCoords.current.scrollLeft;
