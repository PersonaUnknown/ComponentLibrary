...
const Typewriter = forwardRef(({
    ...
    const typePause = () => {
        ...
        if (!loop && displayMode === "multi" && wordIndexRef.current === words.length - 1) {
            return;
     ...
     */
    const getCursor = () => {
        if (displayMode === "single" && (text === children || text === "") && !showCursor) {
            return "";
        }
        ...
        <div className="flex justify-center items-center min-h-6">
            {text}
            {
                ...
                <span 
                    className={twMerge(
                        "inline-block",
                        stateRef.current !== "pause" ? "opacity-100" : blinkCursor ? "animate-blink" : "opacity-100",
                    )}
                >
                    {getCursor()}
                </span>
