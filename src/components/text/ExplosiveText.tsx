...
const ExplosiveText = ({
    ...
    const [randomTransforms, setRandomTransforms] = useState<Transform[]>(new Array(children.length).fill({}));
    ...
    const explodeEffect = () => {
        ...
        for (const _ of children) {
            ...
            const randomX = Math.floor(Math.random() * 20) * (Math.round(Math.random()) * 2 - 1);
            const randomY = Math.floor(Math.random() * 20) * (Math.round(Math.random()) * 2 - 1);
            newTransforms.push({
    ...
    return (
        <div className={className} onMouseOver={onMouseOver} onMouseLeave={onMouseLeave}>
            {children.split("").map((char, index) => {
                ...
                const transform = index < 0 || index >= randomTransforms.length ? 
                    {
                        rotation: 0,
                        translateX: 0,
                        translateY: 0
