 ...
 */
const CarouselDemo = () => {
	const words = ["Hello", "World", "Final Page :)"];
	const carouselRef = useRef<CarouselRef>(null);
	return (
		<div>
			<div className="min-h-screen p-4 md:p-6 manrope">
				<BackButton />
				<h1 className="text-center text-2xl sm:text-3xl font-bold">Carousel</h1>
				<h2 className="text-center text-sm sm:text-base">
					Displays content one at a time. Can traverse with mouse / touch
					controls or button input. Dragging snaps carousel to the nearest
					carousel content.
				</h2>
					...
					<div className="w-full md:w-3xl lg:w-5xl mx-auto">
						<DemoCard
							...
							child={
								<Carousel>
									{words.map((word) => {
										return <ExampleCard key={word}>{word}</ExampleCard>;
									})}
								</Carousel>
							}
						/>
						<DemoCard
							...
							child={
								<Carousel className="w-72 bg-red-500 border-0 rounded-2xl space-mono">
									{words.map((word) => {
										return <ExampleCard key={word}>{word}</ExampleCard>;
									})}
								</Carousel>
...
const ExampleCard = ({ children }: Props) => {
	return (
		<div className={"feature-row w-full p-8 shrink-0 border border-[#e6e6e6]"}>
			<div>
				...
				<p className="text-xs sm:text-sm md:text-lg lg:text-2xl 2xl:text-3xl text-[#666680]">
					Insert Text Here
				</p>
			</div>
		</div>
